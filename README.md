...
## TL&DR
---
Kubernetes has been created by the project called noobspray which is inspired by kubespray. The cluster has been created with 4 nodes. 1 master and 3 workers. Then ArgoCD has been installed to that cluster with helm install. After the installation of ArgoCD, I have created an applicationset.yaml which will be creating the applications based on the current structure of the gitops folder. The applicationset.yaml will be creating the all applications.
